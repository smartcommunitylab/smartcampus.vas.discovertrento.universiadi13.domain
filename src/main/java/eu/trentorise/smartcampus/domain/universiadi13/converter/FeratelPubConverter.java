...
public class FeratelPubConverter implements DataConverter {
	...
	public Object fromMessage(Serializable object) {
		...
		for (ByteString bs : data) {
			try {
				StrutturaPubblica org = StrutturaPubblica.parseFrom(bs);
	...
	private GenericPOI extractGenericPOI(StrutturaPubblica venue) throws ParseException {
		GenericPOI gp = new GenericPOI();
		
		gp.setType(TYPE_FERATEL + " - Pubblica");
