...
public class FeratelRistConverter implements DataConverter {
	...
	public Object fromMessage(Serializable object) {
		...
		for (ByteString bs : data) {
			try {
				StrutturaRistoro org = StrutturaRistoro.parseFrom(bs);
	...
	private GenericPOI extractGenericPOI(StrutturaRistoro venue) throws ParseException {
		GenericPOI gp = new GenericPOI();
		
		gp.setType(TYPE_FERATEL + " - Ristoro");
		...
		if (venue.hasCucina()) {
			map.put("cucina", venue.getCucina());
		}
